...
class ExtractedCriterion(BaseModel):
    """A single extracted criterion with text, type, and confidence."""
...
class ExtractionResult(BaseModel):
    """Result containing a list of extracted criteria."""

    criteria: list[ExtractedCriterion] = Field(default_factory=list)


async def test_structured_output() -> None:
    """Test structured output extraction with Model Garden."""
    cfg = AgentConfig.from_env()
    # Ensure high tokens
    os.environ["MEDGEMMA_MAX_TOKENS"] = "1024"
    cfg = AgentConfig.from_env()

    print("Testing with_structured_output for Model Garden with Robust Parsing...")
    ...
    messages = [
        SystemMessage(
            content=(
                "You are a JSON extractor. Output ONLY JSON. "
                "No thought process. No markdown. Match the schema exactly."
            )
        ),
        HumanMessage(
            content=(
                "Protocol: Children under 18 years old are excluded. "
